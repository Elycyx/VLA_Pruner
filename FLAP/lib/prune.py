...
def prepare_calibration_input(model, dataloader, device, seqlen):
    ...
    Args:
        ...
        seqlen (int): Sequence length for input tensors.
    ...
    inps = torch.zeros((2048, seqlen, model.config.hidden_size), dtype=dtype, device=device)
...
def prune_flap(args, model, tokenizer, device=torch.device("cuda:0")):
    ...
    with torch.no_grad():
        inps, outs, attention_mask, position_ids = prepare_calibration_input(model, dataloader, device, 128)
    layers = model.model.layers
...
def prune_with_predefined_ratio(args, model, tokenizer, layer_ratios: List[float], device=torch.device("cuda:0")):
    """
    使用预定义的每层剪枝比例进行剪枝
    ...
    Args:
        ...
        model: 要剪枝的模型
        tokenizer: 分词器
        layer_ratios: List[float], 每层的剪枝比例列表
        device: 运行设备
    """
    use_cache = model.config.use_cache 
    model.config.use_cache = False 
