...
from lib.prune import prune_wanda_sp, prune_flap, prune_magnitude_sp, check_sparsity, prune_with_predefined_ratio
...
def get_vla_llm(model, cache_dir="llm_weights"):
    for i in range(32):
        model.model.layers[i].self_attn.o_proj.bias = torch.nn.Parameter(torch.zeros_like(model.model.layers[i].self_attn.o_proj.bias, device='cpu'))  # 或 'cuda'
        model.model.layers[i].mlp.down_proj.bias = torch.nn.Parameter(torch.zeros_like(model.model.layers[i].mlp.down_proj.bias, device='cpu'))  # 或 'cuda'
        torch.nn.init.zeros_(model.model.layers[i].self_attn.o_proj.bias)
...
def main():
    ...
    parser.add_argument('--layer_ratios', type=str, default=None, 
                        help='Comma-separated pruning ratios for each layer, e.g., "0.2,0.4,0.1"')
    ...
    if args.layer_ratios is not None:
        # 将字符串转换为浮点数列表
        layer_ratios = [float(x) for x in args.layer_ratios.split(',')]
        prune_with_predefined_ratio(args, model, tokenizer, layer_ratios, device)
    else:
        # 原有的剪枝逻辑
