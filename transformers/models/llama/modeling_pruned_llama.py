...
import math
from typing import List, Optional, Tuple, Union
from .modeling_llama import repeat_kv, apply_rotary_pos_emb
...
from .configuration_pruned_llama import PrunedLlamaConfig
...
class PrunedLlamaAttention(LlamaAttention):
    """
    Pruned version of LlamaAttention where the feature dimensions are specified per layer.
    Inherits from LlamaAttention but uses layer-specific attention feature sizes.
    """
    def __init__(self, config: PrunedLlamaConfig, layer_idx: Optional[int] = None):
        ...
        self.k_proj = nn.Linear(self.hidden_size, attention_feature_size, bias=config.attention_bias)
        self.v_proj = nn.Linear(self.hidden_size, attention_feature_size, bias=config.attention_bias)
        self.o_proj = nn.Linear(attention_feature_size, self.hidden_size, bias=config.attention_bias)

        self.rotary_emb = LlamaRotaryEmbedding(config=self.config)


class PrunedLlamaFlashAttention2(PrunedLlamaAttention):
    """
...
class PrunedLlamaSdpaAttention(PrunedLlamaAttention):
    """
...
class PrunedLlamaDecoderLayer(LlamaDecoderLayer):
    """
    Pruned version of LlamaDecoderLayer where the feature dimensions are specified per layer.
    Inherits from LlamaDecoderLayer but uses PrunedLlamaAttention and layer-specific sizes.
    """
...
class PrunedLlamaMLP(LlamaMLP):
    """
    Pruned version of LlamaMLP where the intermediate size is specified per layer.
    Inherits from LlamaMLP but uses layer-specific intermediate sizes.
    """
    def __init__(self, config: PrunedLlamaConfig, layer_idx: int):
...
class PrunedLlamaModel(LlamaModel):
    """
    Pruned version of LlamaModel where each layer can have different feature dimensions.
    Inherits from LlamaModel but uses PrunedLlamaDecoderLayer.
    """
...
class PrunedLlamaForCausalLM(LlamaForCausalLM):
    """
    Pruned version of LlamaForCausalLM.
    Inherits from LlamaForCausalLM but uses PrunedLlamaModel.
    """
